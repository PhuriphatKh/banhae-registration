...
function EditUserProfile() {
  ...
  const subjects = useMemo(() => {
    return (rawSubjects || []).slice().sort((a, b) => {
      const valA = a.classLevel || "";
      const valB = b.classLevel || "";
  ...
  // Reuse urls
  const PROVINCE_URL = import.meta.env.VITE_PROVINCE_URL;
  const AMPHURE_URL = import.meta.env.VITE_AMPHURE_URL;
  const TAMBON_URL = import.meta.env.VITE_TAMBON_URL;
  ...
  useEffect(() => {
    ...
    fetchJSON(TAMBON_URL)
      .then((data) => {
        if (!mounted) return;
        const filtered = data.filter(
          (d) => d.district_id === parseInt(regDistrict)
        );
    ...
    fetchJSON(TAMBON_URL)
      .then((data) => {
        ...
        if (selectedSubDistrict) {
          setRegZipcode(selectedSubDistrict.zip_code);
        }
      })
  ...
  const handleConfirmSave = async () => {
    try {
      ...
      // นำทางไปยังหน้าโปรไฟล์ของ id ที่บันทึก
