...
function StudentRecords() {
    ...
    const unsub = onSnapshot(collection(db, "approval_requests"), (snap) => {
      ...
      snap.docs.forEach((d) => {
        ...
        if (!map[sid]) {
          ...
          // keep explicit "not_approved" or "canceled" if already set (they should force zero)
        }
      });
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(colRef, (qSnap) => {
      const rows = qSnap.docs
        .map((d) => {
          ...
          // check approval status for this subject/year/class/student
          const subjectId = data.id ?? d.id;
          const statusForThis = approvalStatusMap[subjectId] || null;

          // treat only 'approved' or 'pending' as allowed; explicitly treat
          // 'not_approved' or 'canceled' (and any other) as NOT allowed -> show 0
          const hasApprovedOrPending =
            statusForThis === "approved" || statusForThis === "pending";
          const isExplicitlyRejected =
            statusForThis === "not_approved" || statusForThis === "canceled";

          const studentCopy = { ...stu };

          // if there's no approval or it's explicitly rejected/canceled -> set zeros
          if (!hasApprovedOrPending || isExplicitlyRejected) {
            if (studentCopy.grandTotal != null) studentCopy.grandTotal = 0;
            if (studentCopy.result != null) studentCopy.result = 0;
          }

          // copy scoring_criteria and force grandTotal = 0 when not approved
    ...
    return unsubscribe;
  }, [db, classLevel, studentId, selectedYear, pendingSubjects, approvalStatusMap]);
